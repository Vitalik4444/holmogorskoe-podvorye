...
Сервер сам проверяет репозиторий раз в пять минут и подтягивает новые коммиты.
Опрос, а не вебхук: серверу не нужен открытый наружу порт, GitHub не нужен доступ
внутрь, и нечему протухать вроде токена. `git fetch` без изменений стоит доли
секунды, а при отсутствии коммитов скрипт сразу выходит и nginx не трогает.

```
cp /var/www/podvorye.com/deploy/podvorye-update.service /etc/systemd/system/
cp /var/www/podvorye.com/deploy/podvorye-update.timer   /etc/systemd/system/
systemctl daemon-reload
systemctl enable --now podvorye-update.timer
```

Проверка:

```
systemctl list-timers podvorye-update      # когда следующий запуск
journalctl -u podvorye-update -n 30        # что делал в прошлые разы
```

Выключить на время (например, чтобы править конфиг руками):

```
systemctl stop podvorye-update.timer       # до перезагрузки
systemctl disable --now podvorye-update.timer   # насовсем
```

Важно: скрипт делает `git reset --hard`, то есть правки, сделанные прямо
в `/var/www/podvorye.com`, затрутся при следующем запуске. Менять сайт можно
только через репозиторий — это и задумано, но при отладке на сервере таймер
лучше остановить.

---

## 8. Обновление сайта вручную
...
На сервере ничего делать не нужно — таймер подхватит правку в течение пяти минут.
Если ждать не хочется, ту же команду можно дёрнуть сразу, прямо с рабочей машины:

```
ssh root@161.104.16.231 'bash /var/www/podvorye.com/deploy/update.sh'
```
